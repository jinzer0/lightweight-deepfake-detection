...
def parse_args(argv=None):
    p=argparse.ArgumentParser(description='Train CLIP-only GenImage classifier.')
...
def main(argv=None):
    ...
    output_dir=args.output_dir or cfg.get('paths',{}).get('checkpoint_dir','artifacts/checkpoints/clip_only')
    train_x, train_y = load_split_features(feature_dir, 'clip', 'train')
    val_x, val_y = load_split_features(feature_dir, 'clip', 'val')
    model=ClipClassifier(input_dim=int(train_x.shape[1]), hidden_dim=int(cfg.get('classifier',{}).get('hidden_dim',512)), dropout=float(cfg.get('classifier',{}).get('dropout',0.2)))
